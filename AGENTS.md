...
- `make build` builds all 7 `cmd/` binaries into `bin/`. The Docker image ships only `validator` and `landscape-updater`; CI runs `validator`. The rest (`bootstrap`, `audit-checker`, `staleness-checker`, `generate-schema`, `migrate`) are manual-use tools.
