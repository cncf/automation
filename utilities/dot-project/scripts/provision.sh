...
create_onboarding_issue() {
    ...
    # Maintainer suggestions from org governance files. bootstrap writes these to
    # a gitignored .cache file (so they are never committed to the repo); we read
    # them here purely to surface them in the onboarding issue.
    local suggestions_block=""
    if [[ -f "${tmp_dir}/.cache/maintainer-suggestions.md" ]]; then
        suggestions_block=$'\n'"$(cat "${tmp_dir}/.cache/maintainer-suggestions.md")"$'\n'
    fi
