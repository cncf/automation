...
func TestIdentityTypeValidation(t *testing.T) {
	tests := []struct {
		...
		{"cla only exception", &IdentityType{HasCLA: true, CLAOnly: true}, false, ""},
		{"cla only with url", &IdentityType{HasCLA: true, CLAOnly: true, CLAURL: &PathRef{Path: "https://example.com/cla"}}, false, ""},
		{"cla only without has_cla", &IdentityType{CLAOnly: true}, true, "cla_only requires has_cla"},
		{"cla only with dco", &IdentityType{HasDCO: true, HasCLA: true, CLAOnly: true}, false, ""},
