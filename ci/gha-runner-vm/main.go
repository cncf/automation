...
func run(cmd *cobra.Command, argv []string) error {
	...
	if args.arch == "arm64" {
		...
		command = exec.Command("oci", "raw-request", "--http-method", "POST", "--target-uri", "https://iaas.us-sanjose-1.oraclecloud.com/20160918/computeImageCapabilitySchemas", "--request-body", "file://capability-update.json", "--config-file", "/home/ubuntu/.oci/config", "--profile", "DEFAULT")
		...
		command = exec.Command("oci", "raw-request", "--http-method", "PUT", "--target-uri", "https://iaas.us-sanjose-1.oraclecloud.com/20160918/images/" + imageID + "/shapes/BM.Standard.A1.160", "--request-body", "{\"imageId\":\"" + imageID + "\",\"shape\":\"BM.Standard.A1.160\"}", "--config-file", "/home/ubuntu/.oci/config", "--profile", "DEFAULT")
		output, err = command.CombinedOutput()
		if err != nil {
			log.Print(command.String())
			log.Printf("OCI command failed. Output:\n%s", string(output))
		...
		for _, machine := range removeList {
			command = exec.Command("oci", "raw-request", "--http-method", "DELETE", "--target-uri", "https://iaas.us-sanjose-1.oraclecloud.com/20160918/images/" + imageID + "/shapes/" + machine, "--request-body", "{\"imageId\":\"" + imageID + "\"}", "--config-file", "/home/ubuntu/.oci/config", "--profile", "DEFAULT")
			output, err := command.CombinedOutput()
			if err != nil {
				log.Print(command.String())
				log.Printf("OCI command failed. Output:\n%s", string(output))
