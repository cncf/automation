...
for VAR_NAME in $MANDATORY_VARS; do
  # Verwende indirect expansion, um den Wert der Variable zu bekommen, deren Name in VAR_NAME steht.
  # Die Konstruktion ${!VAR_NAME:-} prüft, ob die Variable gesetzt ist, ohne bei "set -u" sofort abzubrechen, falls sie leer ist.
  if [ -z "${!VAR_NAME:-}" ]; then
...
SBOM_OUTPUT_FILE="${WORKSPACE_DIR}/sbom.json" # Adjusted
...
echo "${SYFT_BIN}" scan "dir:${REPO_DIR}" -o spdx-json="${SBOM_OUTPUT_FILE}" --enrich all --source-name ${GITHUB_REPO_SHORT_NAME} --source-version ${GIT_BRANCH_OR_TAG}"
...
if [ -s "${SBOM_OUTPUT_FILE}" ]; then # Only ingest if SBOM is not empty
  echo "${GUACONE_BIN}" collect files --gql-addr "${GUAC_GQL_ADDR}" "${SBOM_OUTPUT_FILE}" --add-depsdev-on-ingest  --add-eol-on-ingest --add-vuln-on-ingest
